...
export default function AuthPage() {
    ...
    const handleAction = async (type: "send" | "signup" | "login") => {
        setLoading(true);
        try {
            const endpoint = type === "login" ? "/api/login" : type === "send" ? "/api/otp/send" : "/api/otp/verify";
                ...
                <div className="bg-[#111111]/80 backdrop-blur-3xl border border-white/10 rounded-[40px] p-8 sm:p-10 shadow-2xl">
                    ...
                    <div className="flex p-1.5 bg-black/40 rounded-2xl mb-8 border border-white/5">
                        {(["login", "signup"] as AuthMode[]).map((m) => (
                            <button
                                key={m}
                                onClick={() => { setMode(m); setStep("email"); setPin(""); }}
                                className={`flex-1 py-3 text-sm font-bold rounded-xl transition-all cursor-pointer ${mode === m ? "bg-orange-600 text-white shadow-lg" : "text-gray-500 hover:text-gray-300"}`}
                            >
                    ...
                    <div className="space-y-6">
                        <AnimatePresence mode="wait">
                            <motion.div key={mode + step} initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} className="space-y-5">
                                {mode === "signup" && step === "email" && (
                                    <Field label="Таны нэр">
                                        <Input icon="👤" placeholder="Нэрээ оруулна уу" value={name} onChange={e => setName(e.target.value)} />
                                    </Field>
                                )}

                                <Field label="Имэйл хаяг">
                                    <Input icon="📧" type="email" placeholder="email@address.com" value={email} onChange={e => setEmail(e.target.value)} />
                                </Field>
                                ...
                                {step === "verify" && (
                                    <Field label="Баталгаажуулах код">
                                        <Input icon="🔑" placeholder="000000" value={otp} onChange={e => setOtp(e.target.value)} maxLength={6} className="text-center tracking-widest font-mono" />
                                    </Field>
                                )}
                            </motion.div>
                        </AnimatePresence>
                        ...
                        >
                            {mode === "login" ? "Нэвтрэх" : step === "email" ? "Үргэлжлүүлэх" : "Бүртгэл дуусгах"}
                        </PrimaryBtn>
                    </div>
                </div>
            </motion.div>
...
function Field({ label, children }: { label: string; children: React.ReactNode }) {
    return (
        <div className="space-y-2">
            <label className="text-[10px] font-black uppercase tracking-widest text-gray-600 ml-1.5">{label}</label>
            {children}
        </div>
    );
}

function Input({ icon, className, ...props }: ComponentPropsWithoutRef<"input"> & { icon: string }) {
    return (
        <div className="relative group">
            <span className="absolute left-4 top-1/2 -translate-y-1/2 text-sm opacity-40 group-focus-within:opacity-100 transition-opacity">{icon}</span>
            <input {...props} className={`w-full bg-white/5 border border-white/10 rounded-2xl py-4 pl-11 pr-4 text-sm outline-none focus:border-orange-500/50 focus:bg-white/10 transition-all placeholder:text-gray-700 text-white ${className || ""}`} />
        </div>
    );
}
...
function PrimaryBtn({ children, loading, disabled, onClick }: { children: React.ReactNode; loading: boolean; disabled: boolean; onClick: () => void }) {
    return (
        <button
            ...
            className={`w-full py-4 rounded-2xl font-black text-sm transition-all duration-300 flex items-center justify-center gap-3 shadow-xl cursor-pointer ${disabled || loading ? "bg-gray-800/50 text-gray-600 cursor-not-allowed" : "bg-gradient-to-r from-orange-500 to-orange-600 text-white hover:scale-[1.02] active:scale-[0.98] shadow-orange-500/20"}`}
        >
